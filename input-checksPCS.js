...
const checkVariableValidity = (globalParams, thisTradeParams) => {
    // yes this is ugly but we need to check that people aren't entering retarded inputs
        ...
        if (!isNumberMoreThanEq(globalParams._keepBNB, 0.1)) {
            isBadVariable(`_keepBNB`, `Needs to be 0.1 or higher to pay for gas`);
            return false;
        }
        if (!isNumberMoreThanEq(globalParams._minBnbToTrade, 0.01)) {
