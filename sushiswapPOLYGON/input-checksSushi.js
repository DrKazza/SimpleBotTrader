...
const checkVariableValidity = (globalParams, thisTradeParams) => {
    // yes this is ugly but we need to check that people aren't entering retarded inputs
        ...
        if (!isNumberMoreThanEq(globalParams._keepMATIC, 0.1)) {
            isBadVariable(`_keepMATIC`, `Needs to be 0.1 or higher to pay for gas`);
            return false;
        }
        if (!isNumberMoreThanEq(globalParams._minMATICToTrade, 0.1)) {
            isBadVariable(`_minMATICToTrade`, `Needs to be 0.1 or higher to prevent worthless trades`);
            return false;
        }
        if (!isNumberMoreThanEq(globalParams._haltOnLowMATIC, 0.1)) {
            isBadVariable(`_haltOnLowMATIC`, `Needs to be 0.1 or higher so you don't dry out your wallet`);
            return false;
        }
...
const verboseTradeDescription = async (globalParams, thisTradeParams, initialPrice) => {
    ...
    msg5 = `Always keeping approx ${globalParams._keepMATIC} MATIC in the wallet for gas etc.`;
